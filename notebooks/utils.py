...
def threshold_df_correlation(df, thresh=0.3, 
                             use_absolute_corr=True, show_corrs=False):
    """
...
def find_matching_variables(df, substring):
    """
    find variables in a data frame that contain a substring
    """
    return([i for i in df.columns if i.find(substring) > -1])
